...
public class AdminDaoTest {
	...
	public void insertCredential() {
		...
		byId.getAdminName();
		byId.setCredential(new Credential("ashishact2021","ashish123@",Role.ADMIN));
		adminRepo.save(byId);
		
		Admin byId2 = adminRepo.findById(2).get();
		byId2.getAdminName();
		byId2.setCredential(new Credential("rishavact2021","rishav123@",Role.ADMIN));
		adminRepo.save(byId2);
		
		Admin byId3 = adminRepo.findById(3).get();
		byId3.getAdminName();
		byId3.setCredential(new Credential("abhilashact2021","abhilash123@",Role.ADMIN));
		adminRepo.save(byId3);
		
		Admin byId4 = adminRepo.findById(4).get();
		byId4.getAdminName();
		byId4.setCredential(new Credential("nekRanjanact2021","nekranjan123@",Role.ADMIN));
		adminRepo.save(byId4);
		
		Admin byId5 = adminRepo.findById(5).get();
		byId5.getAdminName();
		byId5.setCredential(new Credential("chiragact2021","chirag123@",Role.ADMIN));
